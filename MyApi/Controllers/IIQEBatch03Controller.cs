...
namespace MyApi.Controllers
{
    ...
    [ApiController]
    public class IIQEBatch03Controller : Controller
    {
        private const string LogFilePath = @"C:\LogFileOpenExe\log.txt";

        [HttpGet]
        public IActionResult IIQEBatch03()
        {
            try
            {
                // Log or return message indicating work started
                //Console.WriteLine("IIQEBatch01!!");
                LogMessage("IIQEBatch03!!");
                string exePath = @"C:\inetpub\wwwroot\IIQE Batch\03 IIQE_SEND_C_EXAM_ROUND\IIQE_SEND_C_EXAM_ROUND.exe";
