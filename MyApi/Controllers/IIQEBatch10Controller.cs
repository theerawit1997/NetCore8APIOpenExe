...
namespace MyApi.Controllers
{
    ...
    [ApiController]
    public class IIQEBatch10Controller : Controller
    {
        private const string LogFilePath = @"C:\LogFileOpenExe\log.txt";

        [HttpGet]
        public IActionResult IIQEBatch10()
        {
            try
            {
                // Log or return message indicating work started
                //Console.WriteLine("IIQEBatch01!!");
                LogMessage("IIQEBatch10!!");
                string exePath = @"C:\inetpub\wwwroot\IIQE Batch\FixMigration\Migrate_Data_AG_TO_IIQE.exe";
