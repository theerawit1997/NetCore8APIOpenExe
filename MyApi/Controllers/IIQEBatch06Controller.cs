...
namespace MyApi.Controllers
{
    ...
    [ApiController]
    public class IIQEBatch06Controller : Controller
    {
        private const string LogFilePath = @"C:\LogFileOpenExe\log.txt";

        [HttpGet]
        public IActionResult IIQEBatch06()
        {
            try
            {
                // Log or return message indicating work started
                //Console.WriteLine("IIQEBatch01!!");
                LogMessage("IIQEBatch06!!");
                string exePath = @"C:\inetpub\wwwroot\IIQE Batch\06 IIQE_TRANSFER_P_EXAM_ROUND_TO_AG\IIQE_TRANSFER_P_EXAM_ROUND_TO_AG.exe";
